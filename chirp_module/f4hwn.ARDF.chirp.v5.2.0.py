...
MEM_SIZE =      0x00D00F    # size of all memory
...
ARDF_START =    0x00D000    # ARDF start address
ARDF_STOP =     0x00D010    # ARDF stop address
...
def do_upload(radio):
    ...
    while True:
        if step == 0:
            ...
            start_addr = ARDF_START
            stop_addr  = ARDF_STOP
            status.max = stop_addr - start_addr
            status.cur = 0
            status.msg = "Uploading ARDF settings"
            radio.status_fn(status)

        elif step == 2 and radio.upload_calibration:
...
#                self._upload_calibration = bool(element.value)
            ...
            elif elname == "ARDFNumFoxes":
                _mem.ARDFNumFoxes = element.value
...
# end add menu firmware with version and option display
        ...
        ardf = RadioSettingGroup("ardf", "ARDF")
